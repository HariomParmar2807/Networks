...
int main()
{
	...
	clintListn = socket(AF_INET, SOCK_STREAM, 0); // creating socket which return an integet who work as socket descripter
	memset(&ipOfServer, '0', sizeof(ipOfServer));// initialize ipOfServer
	memset(dataSending, '0', sizeof(dataSending));// initialize data dataSending
	ipOfServer.sin_family = AF_INET;// IPV4 addresses
